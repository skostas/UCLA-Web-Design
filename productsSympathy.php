...
<html lang="en">
<head>
	<title>Flora's Flowers: Sympathy Arrangements</title>
	<meta charset="UTF-8"/>
...
<body>
	...
	<main>
		<!--section title-->
		<h3 id = "sectionTitle"> Sympathy Arrangements</h3>
<?php
	try{ //try to establish connection 
		//open products DB
		$products_db = new SQLite3('products.db');
	}
	catch(Exception $ex){//if fails, catch error and display message
		echo $ex->getMessage(); 
	}
	$statement = "SELECT Images, Description, Price, Inventory,Units_Per_Purchase,Product_Number FROM products WHERE Category ='Sympathy';";
	...
	if($run){//if query successful ?>
		...
		<table>
			...
			<?php
			//represents position of product in row
			$counter = 0; 
			while($row = $run->fetchArray()){//while there are still records to look through
				
				if($counter === 0){//if first product of row ?>
					<!--create new row-->
					<tr>
				<?php } ?>
					<td><figure>
						<a href = <?php echo $row['Images'];?> name = "productPic" title = <?php echo $row['Product_Number'];?> target="_blank" rel="noopener"><image src=<?php echo $row['Images'];?> width ="200" alt=<?php echo $row['Product_Number'];?>></a>
						<figcaption>
						<?php if($row['Units_Per_Purchase'] === 1){//if can purchase by the unit
							echo $row['Description'],'<br/>$',$row['Price'],'/unit';
						}else{//if must buy in bulk
				...
				<?php if ($counter === 5){//if product is last for a row ?>
					<!--end row-->
					</tr>
				<?php
					$counter = -1; 
