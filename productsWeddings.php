...
<html lang="en">
<head>
	<title>Flora's Flowers: Wedding Arrangements</title>
	<meta charset="UTF-8"/>
...
<body>
	...
	<main>
		<!--section title-->
		<h3 id = "sectionTitle"> Wedding Arrangements</h3>
<?php
	...
	$statement = "SELECT Images, Description, Price, Inventory,Units_Per_Purchase,Product_Number FROM products WHERE Category ='wedding';";
	...
	if($run){ //if query was successful ?>
		...
		<table>
			...
			<?php
			//represents products position in a row 
			$counter = 0; 
			while($row = $run->fetchArray()){ //while there are still records to look through
				
				if($counter === 0){//if product is first in a row ?>
					<!--create new row-->
					<tr>
				<?php } ?>
					<td><figure>
				...
				<?php if ($counter === 5){ //if product is last in a row?>
					</tr>
				<?php
					$counter = -1; 
				}
				//incrememnt row position counter
				$counter++;
			} ?>
			</tr>
			</tbody>
