...
<html lang="en">
<head>
	<title>Flora's Flowers: Special Someone Arrangements</title>
	<meta charset="UTF-8"/>
...
<body>
	...
	<main>
		<!--section title-->
		<h3 id = "sectionTitle"> Special Someone Arrangements</h3>
<?php
	try{ //try to establish connection 
		//open products DB
		$products_db = new SQLite3('products.db');
	}
	catch(Exception $ex){ //if fail, catch error and display message 
		echo $ex->getMessage(); 
	}
	$statement = "SELECT Images, Description, Price, Inventory,Units_Per_Purchase,Product_Number FROM products WHERE Category ='special_someone';";
	...
	if($run){ //if query was successful ?>
		...
		<table>
			...
			<?php 
			//represents products position in row, initialized to 0 
			$counter = 0; 
			while($row = $run->fetchArray()){//while there are still more records to look through
				...
				<?php if ($counter === 5){//if product is last in row ?>
					<!--close row-->
					</tr>
				<?php
					$counter = -1; 
