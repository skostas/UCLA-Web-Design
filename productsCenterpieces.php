...
<html lang="en">
<head>
	<title>Flora's Flowers: Centerpiece Arrangements</title>
	<meta charset="UTF-8"/>
...
<body>
	...
	<main>
		<!--section title-->
		<h3 id = "sectionTitle"> Centerpiece Arrangements</h3>
<?php
	try{ //try to establish connection 
		//open products DB
		$products_db = new SQLite3('products.db');
	}
	catch(Exception $ex){ //if fails catch error and disp error message 
		echo $ex->getMessage(); 
	}

	//query db for all fields within centerpiece category 
	$statement = "SELECT Images, Description, Price, Inventory,Units_Per_Purchase,Product_Number FROM products WHERE Category ='centerpiece';";
	$run = $products_db->query($statement);

	if($run){ //if query successfully ran ?>
		<!--products table-->
		<table>
			<tbody>
				
			<?php 
			//represents index of product in a row, initialize to 0 
			$counter = 0; 
			while($row = $run->fetchArray()){ //while there are still records to look through
				
				if($counter === 0){ //if at first product in a row?>
					<tr>
				<?php } ?>
					<td><figure>
						...
						}else{//if must purchase in bulk, display appropriate number of units 
				...
				<?php if ($counter === 5){ //if at last product in a row?>
					</tr>
				<?php
					$counter = -1; 
				}
