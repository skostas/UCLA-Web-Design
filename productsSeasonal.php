...
<body>
	...
	<main>
		<!--section title-->
		<h3 id = "sectionTitle"> Seasonal Arrangements</h3>
<?php
	try{ //try to establish connection 
		//open products DB
		$products_db = new SQLite3('products.db');
	}
	catch(Exception $ex){ //if fails, catch error and disp message 
		echo $ex->getMessage(); 
	}
	$statement = "SELECT Images, Description, Price, Inventory,Units_Per_Purchase,Product_Number FROM products WHERE Category ='seasonal';";
	...
	if($run){ //if query successful?>
		...
		<table>
			...
			<?php
			//represents products position in a row, initialized to 0
			$counter = 0; 
			while($row = $run->fetchArray()){ //while there are still records to look through
				
				if($counter === 0){//if product is first in row ?>
					<!--create new row-->
					<tr>
				<?php } ?>
					<td><figure>
						...
						<?php if($row['Units_Per_Purchase'] === 1){//if can purchase by the unit
							...
							//display number of units per purchase 
				...
				<?php if ($counter === 5){//if product is last for the row ?>
					<!--end row-->
					</tr>
				<?php
					//reset row position counter 
					$counter = -1; 
				}
